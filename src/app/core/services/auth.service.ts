...
export class AuthService {
  ...
  isAuthenticated(token): Observable<boolean> {
    ...
    return this.httpClient.get<boolean>(`${environment.api_url}/auth/authenticate`, headersConfig);
  }
  ...
  resetPassword(body): Observable<{success: boolean}> {
    ...
    return this.httpClient.put<{ success: boolean }>(`${environment.api_url}/users/reset-password`, {password: body.password}, httpOptions);
  }
}
